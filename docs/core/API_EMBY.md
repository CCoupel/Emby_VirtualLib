...
- `RunTimeTicks` → `RuntimeTicks` (ticks 100 ns, direct)
...
GET /emby/Users/{userId}/Items/{itemId}
    ?Fields=Overview,Genres,Studios,ProviderIds,People,Tags,MediaSources,UserData
X-Emby-Token: {api_key}
```

Même structure que ci-dessus pour un seul item, avec champs supplémentaires : `People`, `Taglines`, `RemoteTrailers`.
...
| `Banner` | Bannière | `banner.jpg` |
| `Disc` | Disque / CD | `disc.jpg` |
| `Art` | ClearArt | `clearart.png` |

**Réponse** : binaire image (JPEG ou PNG selon la source). Code 404 si absente.
...
**URL dans les .strm (via proxy local)** :
```
https://{proxyBaseUrl}/virtuallib/proxy/{connectorId}/{libraryId}/{itemId}
```
...
Le `JellyfinConnector` (issue #15) sera quasi identique à `EmbyConnector` avec ces ajustements mineurs.
